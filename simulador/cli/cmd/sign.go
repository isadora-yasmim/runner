...
var signCmd = &cobra.Command{
	...
	Long: `Cria uma assinatura digital simulada via assinador.jar (modo HTTP por padrão).

O assinador.jar é iniciado automaticamente se não estiver em execução.
A validação de parâmetros é feita exclusivamente pelo assinador.jar.
...
Exemplos:
  ...
  assinatura sign -d contrato.pdf -t 1234 --port 9090`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if !response.Success {
			// Parâmetros rejeitados pelo assinador → erro do usuário
...
func init() {
	...
	signCmd.Flags().StringVarP(&signDocument, "document", "d", "", "Documento a ser assinado (caminho ou conteúdo)")
	signCmd.Flags().StringVarP(&signTokenPin, "token-pin", "t", "", "PIN do dispositivo criptográfico (mínimo 4 caracteres)")
